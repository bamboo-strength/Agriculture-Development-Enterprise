...
export default {
  ...
  data() {      const { prefixIcon } = this.$props;
    return {
      showSelector: false,
      current: '',
      mixinDatacomResData: [],
  ...
  methods: {
      ...
      if ((this.value || this.value === 0) && !this.isDisabled(this.value)) {
        ...
        if (strogeValue || strogeValue === 0) {
          ...
          if (this.defItem > 0 && this.defItem < this.mixinDatacomResData.length) {
            defItem = this.mixinDatacomResData[this.defItem - 1][this.map.value];
          }
          defValue = defItem;
        }
        this.emit(defValue);
    ...
    formatItemName(item) {
      ...
      return this.collection.indexOf('app-list') > 0 ? `${item[this.map.text]}(${item[this.map.value]})` : item[this.map.text] ? item[this.map.text] : `未命名${channel_code}`;
    }
  }
};
</script>
